...
void loop()
{
  ...
  if(tagread == true)                 //New tag is read
  {
    ...
    byte target_tag_up[10] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x2, 0x9, 0xB};
    byte target_tag_down[10] = {0x0, 0x0, 0x7, 0x8, 0x4, 0xA, 0x1, 0x6, 0x0, 0x6};
    
    if (memcmp(tagID, target_tag_up, 10) == 0){
      Serial.println("U");  
    } else if (memcmp(tagID, target_tag_down, 10) == 0){
      Serial.println("D"); 
    } else {
      Serial.println("M"); 
    }
